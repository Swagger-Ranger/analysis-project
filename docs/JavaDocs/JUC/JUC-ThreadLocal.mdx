...
再总结来说，threadlocal发生内存泄漏有两个条件：

1. theadlocal变量没有被回收。`ThreadLocal` 对象仍然被 `ThreadLocalMap` 中的 `Entry` 引用着（通过弱引用），因此键不会变为 `null`，此时对应的值对象是不会被清理的。*（严格来讲，这种其实也不能叫内存泄漏，因为内存泄漏是无法访问但也无法回收，此时是可以通过theadlocal对象访问到的）*
2. 线程对theadlocalmap中entry的value的引用没有断开。因为线程存活进而其中的entry数组就一直存活而entry节点对象就会一直存活，而ThreadLocal被清理了就没有了key来引用它，使得value被entry节点持有但却永远也无法再被访问到，这才发生内存泄漏。

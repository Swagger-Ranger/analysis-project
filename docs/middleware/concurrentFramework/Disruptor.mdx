...
```
这里分析一下，核心方法就是其中的`next`方法，中实际只是增加了写的内存屏障来保证写的顺序，实际没有推进游标，只是更新了自己的缓存值`nextValue`，这样消费者是还不能读取到未准备的数据的，但是保证了写入的正确性，因为单线程，只要按照顺序更新，当后面更新逻辑必然是前面的更新已经执行完成，只要顺序是保证的，那结果就一定是对的，这就是为什么要加`cursor.setVolatile(nextValue);`的原因
...
而消费者读取到值，则是要通过publish去获取，这里才更新游标`cursor.set(sequence);`，然后去唤醒阻塞的消费者。
...
因为消费者的逻辑都是通过在`com.lmax.disruptor.BatchEventProcessor#processEvents`中去处理的，其不断的`while (true)`去通过`waitFor`来获取最新可以的槽位数据。
这就是单生产者的协调。
...
**多生产者相对单生产者要复杂一点，但这也是`Disruptor`实现非常非常吊的地方**，消费者中每个`Sequence`是只有一个线程在访问的，
...
多生产者的协调，这里有两个问题是需要理解清楚的：

1. 生产者之间如何协调，各自写各自的数据到对应的槽位，不相互影响
2. 多个生产者在同时写入，而Disruptor都是按游标不断推进读取的，此时如何保证消费者读取到的数据都是正确的

**多生产者的写入：**

多生产者协调多线程写入的数据，是通过在next申请槽位时确定的，也就是next方法中推进cursor是并发安全的，只要申请了槽位，则每个槽位写的数据都是相互独立的，所以可以避免数据相互覆盖的问题。
...
@Override
    public long getHighestPublishedSequence(final long lowerBound, final long availableSequence)
    {
        ...
        {
            // 这里多生产者必须调用isAvailable来判断写入的数据是否就绪
...
而要保证消费者读取到安全的数据则需要额外的机制。就是增加一个和`bufferSize`同等大小的数组`availableBuffer`，在publish时去将数组对应槽位设置为可用来保证数据的安全读取，同时在整个`com.lmax.disruptor.BatchEventProcessor#processEvents`处理逻辑中，通过`getHighestPublishedSequence`方法多生产者就无缝嵌入了其保证的逻辑在里面调用`isAvailable(sequence)`就是在检查数据是否可用。



`availableBuffer`具体的实现原理如下：

**`Disruptor`在处理这个问题时，通过维护一个和`bufferSize`同等大小的数组`availableBuffer`，来记录其元素是否可以被安全读取，并通过位计算来快速判断出要读取数据的位置和对应位置的数据是否准备就绪**
```Java
